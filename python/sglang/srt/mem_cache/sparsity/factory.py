...
def _parse_sparse_config(server_args) -> SparseConfig:
    """Build SparseConfig from server args, reusing the shared JSON parser."""
    algorithm = "quest"
    backend = "flashattention"
    min_sparse_prompt_len = 2048
    sparse_extra_config = {}
    extra_config = parse_sparse_extra_config(
        server_args.hierarchical_sparse_attention_extra_config
    )
